    ...
    if (args.key && args.action) {
        ...
        switch (action) {
            ...
            case 'extractCompany':
                ...
                break;
            case 'clickLinkedInFeedButton':
                sidePanelActions.clickLinkedInFeedButton(args.data.appData.xpaths, function(res) {
                    ipcRenderer.sendToHost(key, res);
                });
                break;
            case 'getCurrentUrl':
...
class SidePanelActions {
    ...
    clickLinkedInFeedButton(xpath, sendResponse) {
        let _this = this;
        try {
            (async function() {
                // Se hace click en el botón "feed"/"home" de LinkedIn.
                _this.clickInputElementByXpathKey(xpath, document, "USPELinkedInFeedButton");
                await _this.Sleep(_this.GetRandom(5000, 7000));
